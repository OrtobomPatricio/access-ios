...
2. Delete any code in `Code.gs` and paste the content of the `backend/Code.gs` file provided in this project.
3. Save the project (Ctrl+S) as "ImagineQrAPI".

## 3. Initialize Headers
1. In the Apps Script editor toolbar, select the function `setupSheet` from the dropdown.
2. Click **Run**.
...
## 5. Add Test Data
**Tab `devices`**:
- A default device is created by `setupSheet`:
  - `device_id`: `DEV_01`
  - `pin`: `1234`
